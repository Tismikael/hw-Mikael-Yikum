...
private:
     ...
     */
     ...
     //Return the parent company that your company belongs 
    Company* helperLargestCompany(Company* company) {
        
        //Base Case: company has no parent
        if (company->parent == nullptr){
            return company;
        }

        //Recursive Case: traverse company hierarchy
        return helperLargestCompany(company->parent);
    }
};
